...
| R57 | **Overdue and Upcoming disagree on the exact instant a snooze lapses.** `OverdueViewModel.makeOverdueRow` suppresses a row while the raw persisted `ScheduledReminder.scheduledFor > now`, treating that timestamp as a literal wall-clock deadline; `UpcomingViewModel.buildRows` instead folds the same value into `engine.nextAllowedSlot(from: target, in: window, …)`, which can resolve to a *later* instant when the raw timestamp falls outside the window's allowed hours/days. A snooze landing outside window hours therefore has the two screens genuinely disagree about whether the contact is still snoozed at a given moment — real, not cosmetic, and both implementations are new in this PR (staged review round 10). Not fixed here: closing it properly means resolving the lapse instant through the engine in both places, and TF-07/PR25 replaces this entire caller-side re-derivation with the joined `Contact ⋈ ScheduledReminder` `ValueObservation` read (R10) that both screens would share — a fix landing now would be rewritten the moment that lands | `OverdueViewModel.swift`, `UpcomingViewModel.swift` | Resolve both screens' snooze-lapse instant through `ReminderEngine.nextAllowedSlot` identically, or — preferably — retire the question entirely once R10's joined observation gives both screens the same persisted row to read. **Delete this row when that lands; do not carry it forward as closed-not-deleted** | TF-07 (PR25) |
