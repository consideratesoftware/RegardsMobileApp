...
1. **Overdue (Home)** — overdue contacts sectioned by priority tier. Row: photo, name, "2 weeks overdue", channel icon (tap = open deep link). Swipe: **Caught up** / **Snooze 1 wk** (shipped as per-row buttons, R52). Footer shows the live next-digest time (from persisted reminders — the shipped hardcoded "6:00 pm" strings are R11). Empty state: "All caught up." No merged-group indicator on this row (staged review round 11 — an earlier draft of this row carried one, directly competing with the name for width): merge provenance lives on the **Merge Duplicates** screen alone, by decision, because it's identity-management information and no other surface asks the user to act on it.
2. **Upcoming** — reminders in the next `digestHorizonDays` (7/14/30, user-set), grouped by day, from **persisted** `ScheduledReminder` rows via `ValueObservation` (R10). Rows show contact, channel, kind tag (birthday/anniversary), scheduled time. Swipe: **Reach out now** (opens deep link + logs interaction + advances cadence) / **Mark caught up**, cadence rows only (the latter shipped as a per-row button, R52) — an occasion row (birthday/anniversary/custom) has no row action at all, since `markCaughtUp`/`SchedulingPass.caughtUp` only ever clear a *cadence* reminder and there is nothing true and useful for the control to do on one (recorded here, staged review round 9, not a new gap: `UpcomingRow.body` already scopes the button to `row.kind == .cadence` for exactly this reason). The former inert nav-bar Horizon control is removed; the real horizon editor lands with persisted Reminder Windows in TF-05.
