...
- Deleted system contacts → set `archivedAt` (never hard-delete; cadence/log history stays for potential re-add). **`.authorized` only.** Under `.limited`, `fetchAllContacts()` returns just the picker-selected subset, so a stored ref outside it isn't evidence of deletion — deselecting a contact from a limited grant (or an `.authorized → .limited` downgrade) must be a no-op, never an archive (§21). Import, refresh, and un-archival still run under `.limited`.
- Changed contacts → refresh `displayName`, `phonesJson`, and `emailsJson`. **`photoRef` refresh is deferred to PR30**, which owns adding the `CNContactImageDataKey` fetch alongside the birthday/date key additions — no `ContactsSource` implementation reads a photo before then, so PR21 has nothing to refresh it from.
