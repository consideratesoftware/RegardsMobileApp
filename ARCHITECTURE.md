...
| R59 | **`SchedulingPass.snooze` can silently overwrite a concurrent `caughtUp` for the same contact.** `snooze(contactId:)`'s R54 precondition read (`contacts.fetch(id:)`) is a genuine suspension point inside the actor, so a `caughtUp(contactId:)` for the same contact can commit its `.userCaughtUp` state transition in that window and then be overwritten by the snooze's `.pending` upsert at the same canonical `cadenceReminderID` — undoing the user's caught-up action for seven days, with no error and nothing on screen to show it happened. Found by staged review round 13 (PR #49); the type's own doc comment previously claimed "no lost update" without qualification, which is true for snooze-vs-snooze (both writes race the same primary key, last writer wins, no duplicate rows) and false for snooze-vs-`caughtUp` (the two writes disagree about the row's *state*, not its identity). That comment is now scoped to the case it actually covers. Unreachable through either shipped caller today: both are main-actor row taps, and either action replaces the row it was invoked from, so no UI path issues the two concurrently. Deliberately not fixed here (owner ruling, 2026-08-20) — a real fix is a conditional compare-and-set write at the repository layer, structural work inside a type TF-07/PR25 deletes outright, and no test is added because a test here would either assert the bug or paper over it | `SchedulingPass.swift` | Resolve it by construction rather than by patching this stub: PR25's reconciliation pass owns reminder-state transitions and must make the caught-up transition win over a racing snooze write (conditional write, or a single serialized owner for the row's state). **Delete this row when that lands** | TF-07 (PR25) |
