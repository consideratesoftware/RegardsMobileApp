...
All Contacts rows use matched zoom navigation on iOS 18+, disabled when
Reduce Motion is enabled. `ContactDetailScreen` is constructed by a factory
...
returning through that path. Overdue and Upcoming rows are the other
exception, and a newer one (round 12, reversing R52): neither pushes to
Contact Detail any more, so matched zoom does not apply to either — see
items 1 and 2 below, and R52, for the tap-preview model that replaced it.
Both screens render via `List(.insetGrouped)` rather than `ScrollView` +
`RegardsCard` (round 12) — `.swipeActions` only works inside `List` — reusing
the app's existing tokens and `SectionHeader` primitive as the `Section`
header content so the sectioned-card look survives the migration; a
`.safeAreaInset(edge: .bottom)` on each `List` keeps the last row and the
Overdue footer clear of the floating/minimizing tab bar, which `List`,
unlike the old `ScrollView`, does not reserve space for on its own.

1. **Overdue (Home)** — overdue contacts sectioned by priority tier. Row: photo, name, "Xd overdue", a decorative channel glyph (it opens nothing by itself — see Tap, below). Swipe right (leading edge): **Caught up**. Swipe left (trailing edge): **Snooze 1 wk**. Both are native `List`/`.swipeActions` (round 12, reversing R52 — see that entry for why per-row buttons came first and why they were undone before shipping), tinted `accentInk`/`muted` to match the checkmark/clock icons they replace, and reachable through the VoiceOver rotor under the same spoken names (`"Mark <name> caught up"`, `"Snooze <name> 1 week"`) the buttons carried. **Tap** previews the channel action pending TF-08: a native `.alert` names the channel and contact from `preferredChannel` (`"Would call Padmé Amidala. Channel actions aren't wired up yet — this previews what tapping will do."`), honest about being a placeholder rather than framed as an error or as unavailable. This replaces the earlier tap-to-Contact-Detail push (round 12); Contact Detail stays reachable through the Contacts tab, and no long-press fallback was added to recover the lost route — nothing in the accessibility suite or this doc depended on it. Footer shows the live next-digest time (from persisted reminders — the shipped hardcoded "6:00 pm" strings are R11). Empty state: "All caught up." No merged-group indicator on this row (staged review round 11 — an earlier draft of this row carried one, directly competing with the name for width): merge provenance lives on the **Merge Duplicates** screen alone, by decision, because it's identity-management information and no other surface asks the user to act on it.
2. **Upcoming** — reminders in the next `digestHorizonDays` (7/14/30, user-set), grouped by day, from **persisted** `ScheduledReminder` rows via `ValueObservation` (R10). Rows show contact, channel, kind tag (birthday/anniversary), scheduled time. A cadence row's second line (`"every 2 weeks"`, etc.) is gone (Sid's call, round 12, after the same screenshot round that reversed R52): `time` already answers "when," so cadence restated it rather than adding a new fact — the same "one fact too many" shape Overdue's own row trimmed in round 11, just reached by a different route (an earlier round-12 call had left it in, reasoning the code alone didn't show the redundancy; the screenshot did). An occasion row's second line (its occasion name) is unaffected — that line is Upcoming's only "why," not a second "when." The spoken label mirrors this exactly (`UpcomingRowState.accessibilityLabel`); `cadenceText` stays a populated field on the row model, just unread by both the view and the label now. Swipe right (leading edge): **Mark caught up**, cadence rows only, native `.swipeActions` (round 12, reversing R52) — an occasion row (birthday/anniversary/custom) has no swipe action at all, since `markCaughtUp`/`SchedulingPass.caughtUp` only ever clear a *cadence* reminder and there is nothing true and useful for the control to do on one (recorded here, staged review round 9; unchanged by round 12 — `UpcomingRow`'s `CaughtUpSwipeAction` still gates on `row.kind == .cadence` for exactly this reason). No swipe-left action: Upcoming has never carried a Snooze control of its own, and round 12 did not add one. **Tap** previews the channel action pending TF-08, identically to Overdue (see item 1 and R52) — this replaces the earlier tap-to-Contact-Detail push; Contact Detail stays reachable through the Contacts tab. The former inert nav-bar Horizon control is removed; the real horizon editor lands with persisted Reminder Windows in TF-05.
...
| R52 | **Closed, staged review round 12 — inverted from its original finding.** Originally: Overdue and Upcoming shipped Caught up / Snooze as real per-row buttons, not the swipe actions §10 describes, because both screens were `ScrollView`/`RegardsCard`-based and SwiftUI's `.swipeActions` only functions inside `List`; recorded then as a deliberate, owner-approved interim deviation, not a defect requiring a fix. Owner decision, round 12: revisit it anyway, before anything shipped to a device — the icon-only row redesign that made three per-row buttons fit (the row-crowding fix this same lane had just landed) was scaffolding for a control layout being deleted, not something worth carrying forward. Both screens now render via `List(.insetGrouped)`, and Caught up / Snooze are real `.swipeActions` — Overdue keeps both (swipe right/left); Upcoming keeps only Caught up, cadence rows only, since it never had a Snooze control of its own. The `List` migration reused the app's existing tokens and the shared `SectionHeader` primitive as each `Section`'s header content, verified by screenshot against the old `RegardsCard` look before proceeding (both screens' resting states and their swipe reveal), and needed one addition `ScrollView` never did: an explicit `.safeAreaInset(edge: .bottom)`, since `List` does not reserve space for the floating/minimizing tab bar the way the old `ScrollView`'s trailing `Color.clear.frame(height: 40)` implicitly assumed a bar that *did* reserve some (found by screenshot at the bottom of the scroll, not by reasoning about the number — the failure mode was content and the Overdue footer rendering underneath the bar). Tap-to-act awaits TF-08: round 12 also replaced both screens' row tap, previously a push to Contact Detail, with a native `.alert` that previews the channel action by name (`"Would call <name>"`, `"Would open WhatsApp with <name>"`) and is explicit that it is not wired yet — seeded here rather than a new R-item since it is the other half of the same swipe redesign, not an unrelated gap. VoiceOver rotor access to the swipe actions, under the same spoken names the buttons carried, and the row's combined name+overdue announcement, are pinned by `RegardsAccessibilityTests` rather than left to reading the modifier chain — see that suite for the specific tests. That pinning is what caught a second, real regression the modifier chain alone did not reveal: `List` does not collapse a `Button`-rooted row to one accessibility element the way the old `ScrollView` did, even with `.accessibilityElement(children: .ignore)`/`.combine` present — Overdue leaked one child, Upcoming leaked four, confirmed by `row.children(matching: .any).count` rather than assumed from the modifier being there. Fixed with `.accessibilityRepresentation`, substituting the row's whole accessibility subtree instead of trying to hide or merge the real one; see `ios/docs/accessibility.md`'s Test patterns §4 for the full investigation and why this is a standing rule for every future `List` row, not an Overdue/Upcoming-specific patch | `OverdueScreen.swift`, `OverdueRow.swift`, `UpcomingScreen.swift`, `Channel.swift` | Closed by this round; wiring the channel-action alert into a real deep link (rather than a preview of one) remains TF-08's, unchanged from what R11/R58 already say | closed |
| R54 | ✅ **Closed, staged review round 11.** `SchedulingPass.snooze` now reads the contact first and rejects (`false`, `@discardableResult`) rather than writing for an untracked or no-cadence contact — closed at the write itself, not by guarding the two callers that happen to exist today. Widened slightly past the original finding's exact wording: `contact.isActive` is checked in the same guard too, since the contact is already in hand and the check costs nothing — both shipped callers already re-fetch and check it themselves, so this is a no-op for them and only matters for the same class of future caller the rest of this fix is about. Original finding, kept for context: neither this §14 PR22 stub nor either shipped caller (`OverdueViewModel.snooze`, `ContactDetailViewModel.snooze`) checked first, unreachable through either caller today (both only offer Snooze for a row already computed as overdue) but real for any future caller reaching `ContactDetailViewModel.snooze()` — `public`, gated only on `isActive` — some other way. `SchedulingPass` now takes a `ContactRepository` dependency it previously deliberately lacked | `SchedulingPass.swift`, `ContactDetailViewModel.swift`, `OverdueViewModel.swift`, `AppEnvironment.swift` | — | closed |
...
| R56 | **Partially closed, staged review round 11 (promoted from deferred).** `overdueSummary` half: fixed. `ContactDetailViewModel.load()` now reads `scheduler.pendingSnoozeDate(contactId:)` — a narrow read added to `SchedulingPass` itself rather than the full `ReminderRepository` dependency this row originally assumed TF-07/PR25 would need to bring first; see that method's own doc comment for why the narrower fix turned out to be enough. `overdueSummary` suppresses overdue state outright while a pending snooze is still in the future, mirroring `OverdueViewModel.makeOverdueRow`'s identical guard, and `snooze()` reloads on success so the screen reflects it immediately instead of waiting on an unrelated trigger. Same class of bug this session found four times over: Overdue claiming "all caught up" on a corrupt row, Upcoming confirming a caught-up that didn't happen, `record()` reporting success after a no-op write, and this — a screen telling a user something false on a success path. Still open, a different gap with a different owner: `nextReminderLabel`'s separately-tracked hardcoded "Today, 6:30 pm" placeholder (R11), untouched by this fix | `ContactDetailViewModel.swift`, `SchedulingPass.swift` | Wire `nextReminderLabel` to a live persisted-reminder read the same way `overdueSummary` now is, once TF-07/PR25 gives this screen (or the app generally) that surface | TF-07 (PR25) for `nextReminderLabel` only |
| R57 | **Cross-lane gate (owner ruling 2026-08-22): TF-06 must not ship notification scheduling until this is resolved.** TF-06 owns the local notification adapter and is queued *before* TF-07, which owns this fix — so without an explicit gate the deviation below stops being a wrong displayed date and becomes a reminder firing outside the user's declared window, which §9 calls the one unbreakable promise. Recorded in TESTFLIGHT_PLAN.md's TF-06 row too, where the implementer will actually meet it. **Overdue and Upcoming disagree on the exact instant a snooze lapses — and neither one is what §9 actually specifies.** `OverdueViewModel.makeOverdueRow` suppresses a row while the raw persisted `ScheduledReminder.scheduledFor > now`, treating that timestamp as a literal wall-clock deadline; `UpcomingViewModel.buildRows` instead folds the same value into `engine.nextAllowedSlot(from: target, in: window, …)`, which can resolve to a *later* instant when the raw timestamp falls outside the window's allowed hours/days. A snooze landing outside window hours therefore has the two screens genuinely disagree about whether the contact is still snoozed at a given moment — real, not cosmetic, and both implementations are new in this PR (staged review round 10). Folded in, staged review round 11: §9 (line 388) documents Snooze itself as window-resolved at write time — `nextAllowedSlot(from: firedAt + 7d, includingContainingSlot: false)` — but `SchedulingPass.snooze` (the PR22 stub) writes a raw `now + 7d` with no `nextAllowedSlot` call at all — two unstated deviations from §9, not one: the missing window resolution, and the anchor itself, which is the instant Snooze is tapped (`clock()`), not §9's `firedAt`. The two coincide only when a reminder is actioned the moment it fires; every delayed tap pushes the snooze further out than §9 specifies; every bit of window resolution described above happens only at read time, in each screen's own re-derivation, which is the root cause the two screens can disagree to begin with. Registering this here rather than leaving it as a §9 comment only, so PR25 inherits it from the gap it actually is, not from a schema note an implementer has to notice contradicts the code on their own. Not fixed here: closing it properly means resolving `scheduledFor` through the engine at the write in `SchedulingPass.snooze` itself, and TF-07/PR25 replaces this entire caller-side re-derivation with the joined `Contact ⋈ ScheduledReminder` `ValueObservation` read (R10) that both screens would share — a fix landing now would be rewritten the moment that lands | `SchedulingPass.swift`, `OverdueViewModel.swift`, `UpcomingViewModel.swift` | Resolve `scheduledFor` through `ReminderEngine.nextAllowedSlot` at the `SchedulingPass.snooze` write itself, matching §9's documented contract, rather than leaving every reader to re-derive it independently — or, preferably, retire the question entirely once R10's joined observation gives both screens the same persisted, already-resolved row to read. **Delete this row when that lands; do not carry it forward as closed-not-deleted** | TF-07 (PR25) |
| R58 | **No global default communication channel.** `UserProfile` holds only onboarding and entitlement state, and the importer derives every `Contact.preferredChannel` as `.phoneCall`/`.email` with no user input. TF-04 collapsed the row's channel icon to three symbols by action type (phone, video, message), so WhatsApp, Signal, Telegram, Messenger, Discord, Instagram, LinkedIn and email all render as one message bubble — the spoken label still names the specific channel, but a sighted user can no longer tell them apart at a glance. A global default makes the bubble unambiguous for the majority of rows. Owner decisions: the default is `.sms`, it lives in **Settings, not onboarding** (onboarding is where users are lost, and at that point they have not seen their contacts or formed a view on how they reach them), and per-contact overrides come separately via TF-09/PR27's `EditContactScreen` | `UserProfile.swift`, `Records.swift` (migration), `SettingsScreen.swift`, `ContactsImporter.swift` | Ship with TF-08/PR26, not before: every channel control is inert ("Open <Channel>, unavailable", no button trait) until PR26 supplies deep-link routing, so a default shipped earlier would configure behaviour that does not exist and would itself be an inert control | TF-08 / PR26 |
| R59 | ✅ **Closed, staged review round 18 — closed on the review's trace, not on this lane's own claim.** `SchedulingPass` serialises mutations per `contactId` (an in-actor lock across `snooze`, `caughtUp`, and `restorePendingAfterFailedCaughtUp`), so no two mutations for one contact overlap and no decision taken before a suspension can be applied after another's write. Round 18 traced the lock race-free end to end and asked for the promotion; the owner granted it. **Why that sequencing mattered:** this row claimed closure three times before — rounds 13, 14 and 15 — and each time the next review found a further interleaving, because the entry described the fix intended rather than the case analysis done. Rounds 14–16's compare-and-set machinery (conditional upsert, state read-back, `.userCaughtUp` marker row, and the two `ReminderRepository` methods serving them) was **removed**, not layered under the lock: it caused three of the four blockers across those rounds, and its deletion resolved two more outright. Regressions revert-verified on both backends — disabling the lock fails them — plus a twelve-way concurrent mixed-mutation test as a deadlock and starvation check. **Load-bearing premise:** the locks are actor state, so they serialise only callers sharing one `SchedulingPass`; `AppEnvironment.scheduler` is a single `let` handed to every screen, and a second instance in production would void the guarantee silently. The residual restore-compensation window is **not** part of this closure — it is R60 | `SchedulingPass.swift`, `Repositories.swift`, `GRDBMinorRepositories.swift`, `MockRepositories.swift` | — | closed |
| R60 | **`restorePendingAfterFailedCaughtUp` can revert a newer catch-up.** `caughtUp` and its later compensating restore are two *separate* lock acquisitions, so R59's per-contact serialisation does not span them: between an action's `caughtUp` and that same action's restore, another action can snooze the contact and catch it up again, and the restore then sets `.pending` over that newer catch-up. Split out of R59 rather than folded into it (owner ruling, staged review round 18) precisely so it cannot hide inside a row marked closed — it is a different shape of bug: not two operations interleaving, but a compensation that cannot tell the row it transitioned from an identical-looking row someone else transitioned since. Narrow in practice: the window is one failed write nested inside another action's full round trip, and the caller-side rule ("call only when `caughtUp` returned `true` for this same action") is the first line of defence. Closing it properly needs a per-write identity the v2 schema does not carry | `SchedulingPass.swift`, `Records.swift` (schema) | Give the compensating write something to match on — a per-write token the caller passes back, or a transition-identity column — so a restore can prove the row is still the one it transitioned. Fold into TF-07/PR25's reconciliation pass, which owns reminder-state transitions outright and can carry that identity by construction. **Delete this row when that lands** | TF-07 (PR25) |
| R61 | **Known-untested paths in TF-04's interaction surface, listed rather than left implicit.** Recorded at the owner's direction (staged review round 19) as a counterweight to this register having over-claimed closure four times: an explicit list of what is *not* covered is worth more than prose, and gives PR25 something concrete to inherit. ~~(a) `OverdueViewModel.snooze` has no post-generation-bump regression.~~ **Closed, round 20**: `snoozeReloadCorrectsLoadStartedAfterGenerationBump`, using the `GatedUpsertReminderRepository` this gap said would have to be built first. (b) `ObservationSubscriptionToken.deinit` is not proven to cancel its subscription Task and unregister the observer — the stated reason the type exists. (c) No end-to-end test drives a real GRDB `observeTracked()` error through to the view models clearing their token and re-subscribing on the next `load()`; `observeTrackedEndsStreamOnRealGRDBError` proves only that the stream finishes. (d) No parity test for a tracked-set emission that changes nothing. ~~(e) `SchedulingPass`'s per-contact lock (R59) is untested under task cancellation.~~ **Closed, round 20**: `cancelledWaiterDoesNotStrandTheLock`. Worth recording that **no code change was needed** — the property already held, because `unlock` hands the lock to the next waiter regardless of that waiter's cancellation state, so the chain still drains. The test was mutation-checked rather than trusted: breaking the hand-off makes the runtime report `SWIFT TASK CONTINUATION MISUSE: lock(_:) leaked its continuation` and the waiters hang, so the test is load-bearing. Do not "fix" the lock's cancellation handling on inspection alone — the missing piece was the proof, not the behaviour | `OverdueViewModel.swift`, `ObservationSubscriptionToken.swift`, `SchedulingPass.swift`, test targets | (a) and (e) are closed (round 20, at this register's own direction). (b)–(d) remain and fold naturally into TF-07/PR25, which replaces the observation and re-derivation paths they cover. **Delete each item as it gains a test; delete the row when all five are gone** | TF-07 (PR25), except (a) and (e) |
