...
| R61 | **Known-untested paths in TF-04's interaction surface, listed rather than left implicit.** Recorded at the owner's direction (staged review round 19) as a counterweight to this register having over-claimed closure four times: an explicit list of what is *not* covered is worth more than prose, and gives PR25 something concrete to inherit. (a) `OverdueViewModel.snooze` has no post-generation-bump regression — the equivalent for `markCaughtUp` exists (`markCaughtUpReloadCorrectsLoadStartedAfterGenerationBump`), but snooze writes through `upsert`, for which no gated fake exists, so the same proof would need one built first. (b) `ObservationSubscriptionToken.deinit` is not proven to cancel its subscription Task and unregister the observer — the stated reason the type exists. (c) No end-to-end test drives a real GRDB `observeTracked()` error through to the view models clearing their token and re-subscribing on the next `load()`; `observeTrackedEndsStreamOnRealGRDBError` proves only that the stream finishes. (d) No parity test for a tracked-set emission that changes nothing. (e) `SchedulingPass`'s per-contact lock (R59) is untested under task cancellation — a cancelled holder must still hand the lock on, or every later caller for that contact stalls | `OverdueViewModel.swift`, `ObservationSubscriptionToken.swift`, `SchedulingPass.swift`, test targets | Close (a) and (e) first: (e) is a liveness risk in shipped code, and (a) is one gated fake away. (b)–(d) fold naturally into TF-07/PR25, which replaces the observation and re-derivation paths they cover. **Delete each item as it gains a test; delete the row when all five are gone** | TF-07 (PR25), except (a) and (e) |
