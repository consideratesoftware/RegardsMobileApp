...
| R59 | ✅ **Closed, staged review round 15** (round 14's fix was incomplete and the row said "closed" when it was not — corrected here rather than left overstating). Two halves, both now covered: `SchedulingPass.snooze` writes through a compare-and-set (`ReminderRepository.upsert(_:ifCurrentStateIs:)`) against the reminder state it observed *before* its first suspension; and `caughtUp` now materializes a `.userCaughtUp` row when no cadence row exists at all. That second half is what round 14 missed: "no row" is the *normal* pre-first-snooze state, since `snooze` is the only writer of a cadence row, so a racing `caughtUp` no-opped, the snooze observed `nil`, the CAS matched `nil` against `nil`, and the stale `.pending` write landed unopposed — suppressing the contact for seven days immediately after the user marked them caught up. `caughtUp` still returns `false` on that path, because its `Bool` means "cleared a pending snooze" and nothing was cleared: reporting `true` would make `restorePendingAfterFailedCaughtUp` compensate for a snooze the user never had. Revert-verified per half, both backends. An ordinary snooze-after-caught-up still succeeds, so only interleavings are rejected, never sequences. **Round 16 addendum — do not simplify the read-back away:** a rejected compare-and-set only means someone else wrote at this id, and who matters. Another `snooze` (two taps on Contact Detail's un-gated button) leaves the row `.pending` and the user's request *did* succeed; a `caughtUp` leaves `.userCaughtUp` and must win. `snooze` therefore re-reads the state on rejection and reports success for `.pending`. Returning a bare `false` instead — which the round-14 fix did — made Contact Detail announce "Couldn't snooze" to VoiceOver for a snooze that had actually happened, and `concurrentSnoozeResolvesToOneRow` did not catch it because it discarded both return values and only counted rows. It now asserts both. | `SchedulingPass.swift`, `Repositories.swift`, `GRDBMinorRepositories.swift`, `MockRepositories.swift` | — | closed |
