...
| R59 | ✅ **Closed, staged review round 14.** `SchedulingPass.snooze` now writes through a compare-and-set (`ReminderRepository.upsert(_:ifCurrentStateIs:)`) against the reminder state it observed *before* its first suspension, so a `caughtUp` landing mid-flight makes the write fail instead of reverting it. An ordinary snooze-after-caught-up still succeeds — it observes `.userCaughtUp` itself — so only interleavings are rejected, never sequences. Revert-verified on both backends: with the previous blind `upsert`, `caughtUpDuringSnoozeInFlightReadSurvives` fails all three assertions on `.grdb` and `.mock` alike. **Correction worth keeping:** the previous revision of this row deferred the bug to PR25 on the claim it was "unreachable through either shipped caller today." That claim was false, and two independent reviewers each found a path — Contact Detail's Caught up and Snooze buttons are both permanently visible with no busy-gating, so a second tap races an in-flight first; and an Overdue snooze followed by an Upcoming/Detail caught-up for the same contact races the identical actor and canonical id across screens, since switching tabs cancels nothing. The error was generalising "a row that disappears when tapped" from the two list screens to Contact Detail, which is not a list and whose buttons do not disappear. Original finding, kept for context: `snooze`'s R54 precondition read is a real suspension point inside the actor, so a concurrent `caughtUp` could commit `.userCaughtUp` and then be overwritten by the `.pending` upsert at the same canonical `cadenceReminderID` — undoing the user's caught-up for seven days, with no error and nothing on screen to show it | `SchedulingPass.swift`, `Repositories.swift`, `GRDBMinorRepositories.swift`, `MockRepositories.swift` | — | closed |
