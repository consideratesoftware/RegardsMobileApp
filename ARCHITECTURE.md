...
| R19 | Root markdown was exempt from link checks, and README referenced nonexistent `docs/DOMAIN_MODEL.md` and `android/` paths | `README.md`; `guards.yml` | README references ✅ **closed by TF-01 slice 1**; root Markdown checks ✅ **closed by GitHub PR #39** | ✅ **closed** |
...
| R31 | Domain coverage floor absent (§13 promises near-total); coverage collected but unenforced | `ios-ci.yml` | ≥95% xccov gate on `Domain/**` | ✅ **closed by GitHub PR #39** |
...
| R42 | `audit-stress.yml` documentation described obsolete PR-trigger behavior | `audit-stress.yml` | Describe the current post-merge, nightly, and release-candidate signals | ✅ **closed by scheduling policy and GitHub PR #39** |
