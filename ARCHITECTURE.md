...
- Deleted system contacts → set `archivedAt` (never hard-delete; cadence/log history stays for potential re-add). **`.authorized` only.** Under `.limited`, `fetchAllContacts()` returns just the picker-selected subset, so a stored ref outside it isn't evidence of deletion — deselecting a contact from a limited grant (or an `.authorized → .limited` downgrade) must be a no-op, never an archive (§21). A `.authorized` fetch that comes back **wholesale empty** while the store previously held active contacts gets the same treatment: it's indistinguishable from Contacts still repopulating mid-restore (§21) rather than proof every contact was deleted, so the sweep is skipped outright rather than archiving the whole address book on a guess. Import, refresh, and un-archival still run under `.limited`.
