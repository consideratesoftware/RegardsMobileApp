...
| R59 | ⚠️ **Partially closed — deliberately not marked closed here.** `SchedulingPass` now serialises mutations per `contactId` (an in-actor lock around `snooze`, `caughtUp`, and `restorePendingAfterFailedCaughtUp`), so no two mutations for one contact overlap and no decision taken before a suspension can be applied after someone else's write. The narrower compare-and-set that rounds 14–16 built for this is **removed**: the lock makes it redundant, and it cost two blockers of its own. **Why this row does not say "closed":** it has claimed closure three times — round 13 ("unreachable through either shipped caller"), round 14 ("✅ Closed"), round 15 ("✅ Closed… revert-verified per half") — and each time the next review found a further interleaving, four rounds running. The pattern was writing the register entry from the fix intended rather than from the case analysis. Serialisation closes the class rather than one pair at a time, and the regressions are revert-verified on both backends, but the closure claim is left for a review to make rather than self-certified a fourth time. **Load-bearing premise:** the locks are actor state, so they serialise only callers sharing one `SchedulingPass`. `AppEnvironment.scheduler` is a single `let` handed to every screen; constructing a second instance in production would void the guarantee silently. Interleavings covered by regressions: caught-up during an in-flight snooze, the same with no pre-existing row, and twelve concurrent mixed mutations completing without deadlock. Original finding: `snooze`'s precondition read is a suspension point inside the actor, so a concurrent `caughtUp` could commit `.userCaughtUp` and be overwritten by the `.pending` upsert at the same canonical `cadenceReminderID` — undoing the user's caught-up for seven days, silently | `SchedulingPass.swift`, `Repositories.swift`, `GRDBMinorRepositories.swift`, `MockRepositories.swift` | — | closed |
