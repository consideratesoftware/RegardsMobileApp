...
| R55 | **`SchedulingPass.snooze` resolves its 7-day push through the device calendar, not the reminder window's own `timeZone`.** A device/window timezone mismatch, or travel mid-session, can land `scheduledFor` an hour off across a DST transition. Deliberately not fixed in TF-04: §7 already records that this §14 PR22 DB-only stub writes a `scheduledFor` that is not slot-start snapped and never touches `ReminderWindow`/`ReminderEngine` at all, so an hour of drift sits inside an approximation that row already declares. Threading the window into the stub would add a dependency PR25 immediately reworks | `SchedulingPass.swift` | PR25 folds snooze into the real engine, which resolves through the window's own timezone by construction; delete this row when it does | PR25 |
