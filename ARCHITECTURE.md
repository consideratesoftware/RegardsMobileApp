...
- **Toolchain:** CI pins the runner's stable Xcode (currently 16.2 with iOS 18.x simulators; simulator pinned to iPhone 16 Pro). `project.yml` declares `xcodeVersion: "26.0"` for local work on Xcode 26. Both are acceptable until Phase 2; before Phase 3, move CI to the Xcode version you will submit with, run the full suite on the current iOS beta, and record the choice in the decisions log (see §21 "OS-beta season").
...
slot       = nextAllowedSlot(window, from: target)      // nil if window has zero capacity
...
5. **Batching = slot-start snapping (decision #30, R6).** All reminders landing in the same window slot share the **same `scheduledFor` = slot start**, so digest grouping is exact-equality by construction and one OS notification per slot exists (`osNotificationId = "digest-{slotStartEpoch}"` for the batch; single-contact slots use `"contact-{uuid}-{kind}"`). Digest copy: *"3 people are overdue: Priya, Alex, Mom."* Tap → Overdue view. Per-contact nags are the #1 reason this category gets silenced.
...
|---|---|---|
| **PR16** | Engine contract fixes: wall-clock slot math, `Date?` return + degenerate handling, wrap-rejection in `ReminderWindow` validation, never-contacted = `?? createdAt`, same-day-late occasion, slot-start snapping in `batch` semantics | R1, R3–R6, R8 closed; all §13 engine edge-case tests green; no force-unwraps in changed paths (R26) |
...
| R5 | **Same-day-late occasion jumps a year.** Install at noon on the birthday → no birthday nudge until next year | `ReminderEngine.swift:244-248` | §9 contract 4: fire at next possible moment today; test | ✅ **closed by PR16** |
