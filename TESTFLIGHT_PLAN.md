...
| TF-04 | READY | TF-02 | Caught up, Snooze, and Log other persist from every surface they appear on (Contact Detail: all three, as buttons; Overdue row actions: Caught up + Snooze; Upcoming row actions: Caught up only — neither list screen has a Log-other row action, and Upcoming ships no Snooze row action either — `.swipeActions` needs `List`, neither screen has one, R52 records the deviation); Snooze via a narrow `SchedulingPass` DB-only stub (single cadence `ScheduledReminder` write, no window/engine/reconciliation — PR25 absorbs it); live lists update via `ContactRepository.observeTracked()`; interaction, action, and mock/GRDB parity tests pass | PR22; R11, R24, R34, R36, R46, R52 |
