...
struct ContactAccessibilityTests {
    ...
    /// No `effectiveLastInteractedAt` construction argument and no "every N
    /// days"/"last contacted…" assertions here any more (staged review
    /// round 11): both were dropped from `AccessibilityContext` and the
    /// label itself — see `accessibilityLabel(context:)`'s own doc comment
    /// for the standing "labels mirror visible content" principle behind
    /// the removal.
    @Test("Overdue label reads as a natural sentence")
    func overdueLabel() {
        let now = Self.date("2026-04-19 14:00")
        let ctx = Contact.AccessibilityContext(now: now, isOverdue: true, overdueDays: 9)
    ...
    func pluralization() {
        let now = Date()
        let ctx1 = Contact.AccessibilityContext(now: now, isOverdue: true, overdueDays: 1)
        let c1 = Self.makeContact(cadence: 1)
        let label1 = c1.accessibilityLabel(context: ctx1)
        #expect(label1.contains("1 day overdue"))

        let ctx0 = Contact.AccessibilityContext(now: now, isOverdue: true, overdueDays: 0)
    ...
    @Test("Untracked contact's label flags state instead of overdue status")
    func untrackedLabel() {
        let now = Date()
        let ctx = Contact.AccessibilityContext(now: now, isOverdue: false, overdueDays: 0)
        let c = Self.makeContact(tracked: false)
        let label = c.accessibilityLabel(context: ctx)
        #expect(label.contains("not tracked"))
        // No "every" assertion here any more (staged review round 11): cadence
        // phrasing was removed from every label, tracked or not, so a check
        // that untracked specifically omits it no longer discriminates
        // anything — see `accessibilityLabel(context:)`'s doc comment.
    }

    @Test("Status chip mirrors the overdue state")
    func statusChip() {
        let now = Date()
        let overdue = Contact.AccessibilityContext(now: now, isOverdue: true, overdueDays: 9)
        let onTime = Contact.AccessibilityContext(now: now, isOverdue: false, overdueDays: 0)
