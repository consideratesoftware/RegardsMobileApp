...
5. **Flake safety (repo law, learned in PRs #11/#12).** No `waitForExistence` on predicate-matched queries; plain element queries for waits, predicates for read-after-known. UI-test changes must name the focused affected regressions run locally; the one-run audit belongs after merges, while repeated 5× stress belongs to nightly automation and pre-release manual dispatch. Neither broad audit is a routine PR requirement. No sleeps as synchronization, no timing-dependent assertions in unit tests.
...
When you name a missing test, specify it concretely enough that a lightweight agent could write it: inputs, expected output, suggested test name. No praise, no diff summary.
