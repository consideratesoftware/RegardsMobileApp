...
5. **Flake safety (repo law, learned in PRs #11/#12).** No `waitForExistence` on predicate-matched queries; plain element queries for waits, predicates for read-after-known. UI-test changes must name the focused affected regressions run locally; repeated 5× stress belongs to post-merge/nightly automation and is not a routine PR requirement. No sleeps as synchronization, no timing-dependent assertions in unit tests.
