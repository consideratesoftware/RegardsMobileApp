...
1. **Process rules first (cheap, absolute).**
   ...
   - UI-test changes ⇒ PR body states audit-stress 5/5 green; absent is a FIX.
...
FIX: (n)
NIT: (max 3)
PROCESS: audit test present? | table row present? | stress-run claimed? | categories untouched?
```
